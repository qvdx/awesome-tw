...
- `maxDurationHours`: teto de duração do ciclo de coleta (ver Ciclo) — **não** é um intervalo de reagendamento.
...
- Se não há nada ativo (ex: sem tropa disponível ainda, ou `maxDurationHours` tão curto que nenhum nível recebeu tropa): tenta de novo em 30 minutos (`NO_ACTIVE_RETRY_MS`), fixo — não depende mais de `maxDurationHours`, já que esse campo agora só controla o teto de duração, não a cadência de retry.
